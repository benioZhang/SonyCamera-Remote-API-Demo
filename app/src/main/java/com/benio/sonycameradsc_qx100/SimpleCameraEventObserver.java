...
public class SimpleCameraEventObserver {
    ...
    public interface ChangeListener {
        ...
        /**
         * Called when the value of "isoSpeedRate" is changed.
         *
         * @param isoSpeedRate
         */
        void onIsoSpeedRateChanged(String isoSpeedRate);

        /**
         * Called when the value of "shutterSpeed" is changed.
         *
         * @param shutterSpeed
         */
        void onShutterSpeedChanged(String shutterSpeed);
    ...
    private String mIsoSpeedRate;

    // Current Shutter Speed value.
            ...
            public void run() {
                ...
                while (mWhileEventMonitoring) {
                    ...
                    try {
                        ...
                        // isoSpeedRate
                        String isoSpeedRate = findIsoSpeedRate(replyJson);
                        Log.d(TAG, "getEvent isoSpeedRate:" + isoSpeedRate);
                        if (isoSpeedRate != null && !isoSpeedRate.equals(mIsoSpeedRate)) {
                            mIsoSpeedRate = isoSpeedRate;
                            fireIsoSpeedRateChangeListener(isoSpeedRate);
                        }

                        // shutterSpeed
                        String shutterSpeed = findShutterSpeed(replyJson);
                        Log.d(TAG, "getEvent shutterSpeed:" + shutterSpeed);
                        if (shutterSpeed != null && !shutterSpeed.equals(mShutterSpeed)) {
                            mShutterSpeed = shutterSpeed;
                            fireShutterSpeedChangeListener(shutterSpeed);
                        }
    ...
    /**
     * Returns the current ISO Speed Rate value.
     *
     * @return
     */
    ...
    /**
     * Returns the current Shutter Speed value.
     *
     * @return
     */
    ...
    /**
     * Notify the change of Shutter Speed.
     *
     * @param shutterSpeed
     */
    private void fireShutterSpeedChangeListener(final String shutterSpeed) {
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
                    mListener.onShutterSpeedChanged(shutterSpeed);
                }
            }
        });
    }
    ...
    /**
     * Notify the change of ISO Speed Rate.
     *
     * @param isoSpeedRate
     */
    private void fireIsoSpeedRateChangeListener(final String isoSpeedRate) {
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
                    mListener.onIsoSpeedRateChanged(isoSpeedRate);
                }
            }
        });
    }
    ...
    private static String findStorageId(JSONObject replyJson) throws JSONException {
        ...
        if (!resultsObj.isNull(indexOfStorageInformation)) {
            JSONArray storageInformationArray = resultsObj.getJSONArray(indexOfStorageInformation);
    ...
    /**
     * Finds and extracts value of ISO Speed Rate from reply JSON data. As for
     * getEvent v1.0, results[29] => "isoSpeedRate"
     *
     * @param replyJson
     * @return
     * @throws JSONException
     */
    private static String findIsoSpeedRate(JSONObject replyJson) throws JSONException {
        String isoSpeedRate = null;
        int indexOfIsoSpeedRate = 29;
        JSONArray resultsObj = replyJson.getJSONArray("result");
        if (!resultsObj.isNull(indexOfIsoSpeedRate)) {
            JSONObject isoSpeedRateObj = resultsObj.getJSONObject(indexOfIsoSpeedRate);
            String type = isoSpeedRateObj.getString("type");
            if ("isoSpeedRate".equals(type)) {
                isoSpeedRate = isoSpeedRateObj.getString("currentIsoSpeedRate");
            } else {
                Log.w(TAG, "Event reply: Illegal Index (29: isoSpeedRate) " + type);
            }
        }
    ...
    /**
     * Finds and extracts value of Shutter Speed from reply JSON data. As for
     * getEvent v1.0, results[32] => "shutterSpeed"
     *
     * @param replyJson
     * @return
     * @throws JSONException
     */
    private static String findShutterSpeed(JSONObject replyJson) throws JSONException {
        String shutterSpeed = null;
        int indexOfShutterSpeed = 32;
        JSONArray resultsObj = replyJson.getJSONArray("result");
        if (!resultsObj.isNull(indexOfShutterSpeed)) {
            JSONObject shutterSpeedObj = resultsObj.getJSONObject(indexOfShutterSpeed);
            String type = shutterSpeedObj.getString("type");
            if ("shutterSpeed".equals(type)) {
                shutterSpeed = shutterSpeedObj.getString("currentShutterSpeed");
            } else {
                Log.w(TAG, "Event reply: Illegal Index (32: shutterSpeed) " + type);
            }
        }
